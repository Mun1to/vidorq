...
| [yt-dlp](https://github.com/yt-dlp/yt-dlp) | Descarga de vídeos de redes, para poder pegar un link en "Copiar un estilo" en vez de un archivo | **Comprobado el 2026-08-22, v2026.8.19**: la rueda de PyPI pesa **3,04 MB** y no arrastra **ninguna dependencia obligatoria**. Licencia **Unlicense** (dominio público, sin obligaciones), y su README lo acota sin ambigüedad: *"The git repository, the PyPI source distribution and the PyPI built distribution (wheel) only contain code licensed under the Unlicense"*. ⚠️ **La trampa**: los ejecutables que ellos empaquetan con PyInstaller **sí** llevan código GPLv3+ y el conjunto queda GPLv3+. Instalar el paquete de pip es seguro para un producto comercial (regla AG); redistribuir su `.exe` NO. Falta decidir aparte lo de los términos de servicio de TikTok e Instagram, que es cosa distinta de la licencia |
